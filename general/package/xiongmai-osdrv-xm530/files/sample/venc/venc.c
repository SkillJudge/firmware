...
XM_S32 SAMPLE_VENC_400W_CLASSIC(int seconds)
{
    ...
    ViChn = 0;
    s32Ret = ViChnConfig(ViChn, VIDEO_ENCODING_MODE_PAL,CAPTURE_SIZE_4M);//�˴����޸�2560*1440
    if (XM_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Start ViChn %d failed!\n", ViChn);
        return XM_FAILURE;
    ...
    ViChn = 1;
    s32Ret = ViChnConfigEx(ViChn, VIDEO_ENCODING_MODE_PAL,CAPTURE_SIZE_D1);//�˴����޸�2560*1440
    if (XM_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Start ViChn %d failed!\n", ViChn);
        return XM_FAILURE;
    ...
    VencChn = 0;
    s32Ret = COMM_VENC_Start(VencChn, PT_H264,VIDEO_ENCODING_MODE_PAL,CAPTURE_SIZE_4M,SAMPLE_RC_CBR);//�˴����޸�2560*1440
    if (XM_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Start VencChn %d failed!\n", VencChn);
        goto END_VI_STOP;
	...
	s32Cnt += 1;
	VencChn = 1;
	s32Ret = COMM_VENC_Start(VencChn, PT_H264,VIDEO_ENCODING_MODE_PAL,CAPTURE_SIZE_D1,SAMPLE_RC_CBR);//�˴����޸�2560*1440
    if (XM_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Start VencChn %d failed!\n", VencChn);
        goto END_VI_STOP;
...
XM_S32 SAMPLE_VENC_720P_SetTitle(int seconds)
{
    ...
    XM_U8 secondPixel[] = {      //2bit模式，每个像素点占用2bit空间
