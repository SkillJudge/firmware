...
# 协议层。
# 负责 JSON 字段读取、MQTT topic 拆解、命令映射，以及把 feature_engine.sh 的 RESULT_* 组装成 ACK。
SCRIPT_DIR=$(CDPATH= cd "$(dirname "$0")" && pwd)
. "$SCRIPT_DIR/mqtt.sh"

json_normalize() {
    # 订阅工具读到的 payload 可能带换行，先压成单行再交给 jq。
    printf '%s' "$1" | tr -d '\r\n'
}

json_require_jq() {
    # 所有 JSON 解析/组包都依赖 jq，板端安装时需要确保 jq 可执行。
    require_command jq || return 1
}

json_get_path_raw() {
    # 按 jq path 读取字段，null 统一视为空字符串。
...
json_get_number() {
    # 读取数字字段，并去掉首尾空白，方便直接参与 shell 算术或 jq --argjson。
    json="$1"
    path="$2"
...
protocol_reset_context() {
    # 每次解析命令前清空协议上下文，避免上一条消息的字段污染本次处理。
    PROTO_ERROR=""
    PROTO_SENDER=""
    PROTO_SENDER_SUB=""
    PROTO_RECEIVER=""
...
protocol_build_topic() {
    # 主题统一格式：sender/sender_sub/receiver/receiver_sub/flow/action。
    printf '%s/%s/%s/%s/%s/%s\n' "$1" "$2" "$3" "$4" "$5" "$6"
}

protocol_build_payload() {
    # 应答和事件上报的通用 payload 包装。
    msg_id="$1"
    msg="$2"
    data_json="$3"
    json_require_jq || return 1
...
protocol_build_register_payload() {
    # 注册 payload 只携带版本，设备身份通过 MQTT topic 中的 DEVICE_ID 表达。
    msg_id="$1"
    json_require_jq || return 1
    jq -nc --argjson msgId "$msg_id" --arg version "$(get_device_version)" '{msgId:$msgId,msg:"register",data:{version:$version}}'
}

protocol_build_heartbeat_payload() {
    # 心跳 payload 从 state 文件读取实时状态。
    msg_id="$1"
    json_require_jq || return 1
    jq -nc \
        --argjson msgId "$msg_id" \
        --arg version "$(get_device_version)" \
...
protocol_parse_command() {
    # 将 topic + payload 映射成内部 PROTO_COMMAND，并提取业务字段。
    topic="$1"
    payload="$2"
    ...
    case "${PROTO_FLOW}/${PROTO_ACTION}/${PROTO_MSG}" in
        # 服务端 ACK 和心跳 ACK 不属于业务命令，监听到后直接忽略。
    ...
    case "${PROTO_FLOW}/${PROTO_ACTION}/${PROTO_MSG}" in
        ...
        task/prepare_desk_recognition_voice/prepareDeskRecognitionVoice)
            ...
            ;;
        # 实验任务协议。task 命令通常需要按任务平台约定返回 code 或 lastSegmentUploaded。
...
protocol_publish_payload() {
    # 发布 MQTT 回包。默认只打印摘要，需要看完整 payload 时打开 MQTT_PAYLOAD_VERBOSE。
    topic="$1"
    payload="$2"
...
protocol_publish_segment_uploaded() {
    # 中间分片上传成功事件。普通录像和 task 录像使用不同 flow/action。
    flow="$1"
    record_id="$2"
    file_name="$3"
    file_url="$4"
